...
export default function Page() {
  ...
  const handleUpdate = async () => {
    ...
    if (imageUpload) {
      ...
      setLoading(false);
      handleClose();
      toast.success("Owner Data is Updated But Image Not Updated", {
        position: "top-right",
        style: {
          background: "green",
          color: "#fff",
  ...
  const deleteItem = async (id,uid,email,password) => {
    // const auth = getAuth();
    // const userRef = firestoreDb.collection('values').doc(uid);
    ...
    // alert(uid);
    // setLoading(true);
    // updatePassword(usern, newPassword).then(() => {
      // Update successful.
    ...
    // const msg =await auth().updateUser(uid, { password: newPassword });
    // console.log(msg);
    //  deleteUserByUid
    // firestoreDb.auth().getUser(uid).delete().then(function() {
    ...
    // });
    // const user = auth.deleteUser(uid);
    // delete(auth.uid);
    ...
    setLoading(false);
    toast.success("Owner Data Deleted", {
      position: "top-right",
      style: {
        background: "green",
        color: "#fff",
  ...
  function deleteHandle(id){
     deleteDoc(doc(firestoreDb, "user", id));
  }
      ...
      <div className="wrapper">
        ...
        <Modal size="lg" show={show} onHide={handleClose}>
          <Modal.Header className="modal-head" closeButton>
            <Modal.Title>Update Owner Details</Modal.Title>
          </Modal.Header>
          <Modal.Body>
            ...
            <div className="row">
              ...
              <div className=" col-6 form-group">
                <label htmlFor="exampleInputEmail1">Old Image</label>
                 <Link href={imgurl} target="_blank"> <Image className="form-control" src={imgurl} style={{width:"200px",height:"100px"}} alt="Old Image" /> </Link>
              </div>
            </div> 
              ...
              <div className="row mb-2">
                ...
                <div className="col-sm-3">
                  <ol className="breadcrumb float-sm-right">
                    ...
                    <li className="breadcrumb-item active">Owner</li>
                  </ol>
                </div>
              </div>
                ...
                <tbody>
                  {records.map((item, id) => (
                    ...
                    <tr key={item.id}>
                      ...
                      <td>
                        <button
                          onClick={() => deleteItem(item.id,item.userId,item.email,item.password)}
                          className="btn btn-dark"
                        >
                          <FaTrash />
                        </button>
